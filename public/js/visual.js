function editPostTitle(postTitle, postID) {
  $('span.error').hide(); // hide any old error messages
  $('#editPostTitleForm').show(); // show the form
  $('#editPostTitleForm input[type=text]').val(postTitle).focus();
  $('#editPostTitleForm input[type=hidden]').val(postID);
}
...
function update(source) {
  ...
  var nodeEnter = node.enter().append("g")
      ...
      .on("click", function(d) {
           if(d.size == 0) {
             // it's a blog post node
             editPostTitle(d.name, d.post_id);
             deletePost(d.post_id);
             console.log(d.name);
